  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (currentState === 'Sign Up') {
          // Create profile image from initials
          ...
          // On login, use user data from response
          // console.log("user is: ", user);
          // localStorage.setItem('firstName', user?.firstName || '');
          // localStorage.setItem('lastName', user?.lastName || '');
          localStorage.setItem('profileImage', profileImage || assets.profile);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="font-bold hover:text-black text-gray-800 active:text-gray-200">
          Login with Google
        </p>
      </div>
    </form>
