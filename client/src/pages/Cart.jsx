...
export default  function Cart(){
  ...
  const getCartData = async()=>{
    ...
    for(const key in cartItems){
      ...
      // console.log("final price ", response.data.product.finalPrice);
    ...
    <div className=' border-t pt-14 mx-2.5'>
      ...
      <div>
       { isLoading ?   ( 
          ...
          cartData.map((item)=>{
            return (
              <div key = {item._id} className='py-4 border-t  border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_o.5fr] items-center gap-4'>
                <a href={`/product/${item._id}`} className='flex items-start gap-6'>
