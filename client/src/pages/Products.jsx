        ...
        >
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            <label className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Electrical & Electronics'} onChange={toggleCategory}/> Electric & Electronic
            </label>
            <label className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Home & Kitchen'} onChange={toggleCategory}/> Home & Kitchen
            </label>
            <label className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Mobile Accessories'} onChange={toggleCategory}/> Mobile & Laptop
            </label>
            <label className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Bag & Luggage'} onChange={toggleCategory}/> Bag & Luggage
            </label>
             <label className='flex gap-2'>
              <input className='w-3' type='checkbox' value={'Fasion'} onChange={toggleCategory}/> Fasion
            </label>
             <label className='flex gap-2'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
          {
            filterProducts.map((item, index)=>( 
              <ProductItem key={index} name={item.name} id={item._id} discount={item.discount} finalPrice={item.finalPrice}  price={item.price} image={item.image}  />
            ))
          }
        </div>
